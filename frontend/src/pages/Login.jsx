...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const ok = response && (response.status === "Success" || response.data?.token || response.success === true);

      if (ok) {
        setUser(response.data?.user || response.user); // Actualiza el contexto
        // El proyecto actual no tiene ruta /dashboard — redirigimos a /home
        navigate('/home');
      } else {
